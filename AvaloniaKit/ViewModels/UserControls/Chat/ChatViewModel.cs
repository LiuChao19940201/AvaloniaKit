...
public partial class ChatViewModel : PageViewModelBase
{
    ...
    {
        // ★ 天气预报入口（排在基金上方）
        new() { Name = "基金自选", Preview = "点击查看自选基金实时净值", Time = "昨天",  Unread = 0, IsFundTracker = true  },
    ...
    private void OpenChat(ChatItemViewModel item)
    {
        ...
        else if (item.IsDouyin)
            WeakReferenceMessenger.Default.Send(new NavigateToDouyinMessage());
