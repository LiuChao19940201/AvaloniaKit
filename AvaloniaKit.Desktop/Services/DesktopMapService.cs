...
// ══════════════════════════════════════════════════════════════════════════════
//  DesktopMapService — 地图覆盖层（Windows Desktop 端）
//  · WebView2 以 HWND 子窗口形式挂到 Avalonia 主窗口，Bounds 跟随客户区，
//    ★ 顶部留出 topOffsetDip（状态栏安全区 + 标题栏），保留 Avalonia 标题栏与返回按钮；
//    NavigateToString 加载共享 HTML（高德 JS API 地图 + 路线规划 + TTS 播报）
//  · 与抖音覆盖层同一套承载模式（独立实例，避免事件串扰）
//  · 依赖系统 WebView2 Runtime（Win10/11 预装）；缺失时静默失败，标题栏返回仍可退出
// ══════════════════════════════════════════════════════════════════════════════
public class DesktopMapService : IMapService
{
    private Task<CoreWebView2Environment>? _envTask;
    private CoreWebView2Controller? _controller;
    private Window? _window;
    private bool _showing;
    private int _showVersion;   // ★ 代数：快速重进时作废旧的异步 Show 流程
    private double _topOffsetDip;

    public event EventHandler? ExitRequested;
    public event EventHandler<string>? MessageReceived;

    public DesktopMapService()
    {
        // ★ 启动预热：WebView2 环境创建耗时秒级，提前到应用启动时后台完成，
        //   首次进地图省掉这段等待（失败静默，进入时会重试）
    ...
    private static async Task<CoreWebView2Environment> CreateEnvAsync()
    {
        ...
        return await CoreWebView2Environment.CreateAsync(null, dataDir, null);
    }
    ...
    private async Task ShowAsync(string html, int version)
    {
        try
        {
            // ★ 全程用局部变量：重进是 Hide()+Show() 连发，Hide 投递的清理会在
        ...
        {
            // WebView2 Runtime 缺失等场景：保持 Avalonia 兜底页
        }
    }
